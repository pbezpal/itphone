...
import static DataTests.OPENSIPS.OPENSIPS_STATUS;
...
public class Test_C_StatusServers {
    ...
    void test_Booster_Status_Server_Contacts(){
        this.TEST_MESSAGE = MonitoringPage.isConnectService(DX500_BOOSTER, DX500_BOOSTER_CONTROL_PORT, labelContacts);
    ...
    void test_Pult_Status_Server_Contacts(){
        this.TEST_MESSAGE = MonitoringPage.isConnectService(DX500_PULT, DX500_PULT_CONTROL_PORT, labelContacts);
        if(this.TEST_MESSAGE != null) this.TEST_STATUS = false;
    }
    ...
    public void getStatusServer(boolean serverStatus, boolean moduleStatusServer, boolean tableStatusServer, String server){
        if(serverStatus && moduleStatusServer && tableStatusServer) TEST_STATUS = true;
            ...
            this.TEST_STATUS = false;
            this.TEST_MESSAGE = "На сервере запущена служба " + server + ", однако в СУ отображается некорреткный статус";
        }else{
            this.TEST_STATUS = false;
    ...
    public void getStatusConverter(boolean serverStatus, boolean moduleStatusConverter, boolean tableStatusConverter, String SMG){
        if(serverStatus && moduleStatusConverter && tableStatusConverter) TEST_STATUS = true;
            ...
            this.TEST_STATUS = false;
            this.TEST_MESSAGE = "На сервере статус SMG: " + SMG + " - disconnected и в СУ отображается статус SMG - disconnected";
        }
    }
}
